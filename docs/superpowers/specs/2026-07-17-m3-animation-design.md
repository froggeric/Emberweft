# M3 — Animation & Realtime Pipeline (Design Spec, v5 revised)

> **Status:** design — for owner approval · Emberweft · 2026-07-17 (v5: round-3 Principal-Engineer review + fourth flam3 source read; v5 corrects two load-bearing math errors found by direct source verification)
...
> **v5 revision notes (round-3, fourth source read).** A direct verification of `interpolation.c` / `flam3.c` / `variations.c` found and fixed two load-bearing math errors and several precision gaps: (a) **`.log` near-degenerate guard** — v3/v4 called it a "determinant guard → whole-xform `.linear` fallback." The source (`interpolation.c:213-215`) has **no determinant, no NaN guard, no COMPAT fallback** — it is a **per-column magnitude** check `log(cxmag[i][col]) < -10` that switches only that column's magnitude from log to linear accumulation (angle still polar). Corrected in §Transition step 6, §Edge handling, §Parity table, §Risks, and plan Task 10. (b) **`hsv_rgb_palette_blend` is LIVE, not dead** — `hsv_circular`/`rgb` palette blend is `rgb_fraction·new_rgb + (1−rgb_fraction)·new_hsv_rgb` with `rgb_fraction = hsv_rgb_palette_blend` (`interpolation.c:383-384,432-433`); it was missing from the data-model field list. Added `hsvRgbPaletteBlend` to the §Prerequisite Flame fields and plan Tasks 2/3/9. (c) **RNG draw order** — flam3 dispatches in **ascending numeric variation-index order** (varFunc built via `for j if var[j]!=0`); Emberweft CPU evaluates in **alphabetical** order (parser-sorted); Metal in **canonical-slot** order. These three agree on the RNG-consuming subset {julia, julian, juliascope, super_shape, wedge_julia} by coincidence, now pinned as a tested invariant (plan Task 6). (d) `establish_asymmetric_refangles` sym gate is `animate==0` only — the padding branch is dead (`padsymflag` hardcoded 0). (e) `get_stagger_coef` `num_xforms` arg = standard xform count (excludes final); active formula makes the LAST xform interpolate first. (f) There is no `spin_inter` helper in master — `sheep_edge` calls helpers inline; both endpoints are rotated.
...
### Transition — `sheep_edge` (full step list; ports `flam3.c:436-491` — VERIFIED: no separate `spin_inter` helper exists in master; `sheep_edge` calls the helpers inline)
...
4. **`establish_asymmetric_refangles(spun, 2)`** (`flam3.c:477`, def `interpolation.c:710`) — writes **only `xform.wind[col]`** (`interpolation.c:759,761`), not coefs. For an xform symmetric on one side / animated on the other (sym = `animate==0`; the `padding==1 && padsymflag` branch is DEAD in master — `padsymflag` hardcoded 0 at `interpolation.c:753`), it stores the symmetric side's column angle `+2π` into `wind`. This anchors the log-rotation unwrap so an animating xform next to a symmetric one does not spin the wrong way at the ±π seam.
...
6. **`flam3_interpolate(spun, 2, smoother(t), stagger, result)`** (`flam3.c:486-487`) where `smoother(t)=3t²−2t³` (`interpolation.c:339-341`). **`.log` matrix branch** (`interpolation.c:657-699`, via `convert_linear_to_polar` + `interp_and_convert_back`): polar decomposition — magnitude interpolated **log-linearly** (`accmag += c[i]*log(cxmag)`, reconverted via `exp`), angle interpolated with the **`wind`-anchored unwrap** (`interpolation.c:293-309`); translation `e,f` linear. **Near-degenerate guard is PER-COLUMN magnitude, NOT a determinant guard** (`interpolation.c:213-215`): for each column, if `log(cxmag[i][col]) < -10` for any cp, that column's magnitude switches to **linear** accumulation (`expmag = accmag`, not `exp(accmag)`); the angle still polar-interpolates; the xform does NOT fall back to `.linear`. There is no NaN guard and no COMPAT fallback in flam3. Post-matrix special case: if all parents' post is identity, result post forced identity.
7. **Palette blend** happens inline in `flam3_interpolate_n` (`interpolation.c:377-427`) on the resolved `palette[256]` RGB arrays of the two endpoints (HSV/hsv_circular; see Palette below). **The `palette_index0/1`, `hue_rotation0/1`, `palette_blend` fields are dead metadata** (written once during sheep_edge setup, never read) — Emberweft does **not** model them. (`hsv_rgb_palette_blend` is NOT in this dead set — it is consumed live as the rgb/hsv mix fraction; see Palette below.)
...
2. **Add `Flame` fields:** `interpolation` (default `.linear`), `interpolationType` (default `.log`), `paletteInterpolation` (default `.hsvCircular`), `hueRotation` (default 0), and `hsvRgbPaletteBlend` (default 0; flam3 `hsv_rgb_palette_blend` — the LIVE rgb↔hsv mix fraction consumed by the hsv_circular/rgb palette blend at `interpolation.c:383-384,432-433`; NOT one of the dead fields below). **Do NOT** add the palette-hack fields (`paletteIndex0/1`, `hueRotation0/1`, `paletteBlend`) — they are dead metadata (F7). **F4:** `hueRotation` is new; the existing `hueShift` (flam3 `hue`) is kept unchanged. **Add `Xform` fields:** `animate` (default 1.0), `padding`, `wind: SIMD2<Double>` (default 0; load-bearing — anchors `.log` unwrap for asymmetric xform pairs). Extend parser + serializer + round-trip tests.
...
| Near-singular affine during `log` morph | constructed degenerate pair | port flam3's **per-column magnitude guard** (`log(cxmag)<-10` → linear magnitude for that column; NOT a determinant guard, NOT a whole-xform `.linear` fallback); frames finite |
...
- Near-singular affine in `log` interp → port flam3's **per-column magnitude guard** (`log(cxmag)<-10` → linear magnitude for that column, `interpolation.c:213-215`; NOT a determinant guard, NOT a whole-xform fallback).
...
- **Near-singular affine NaN in `log` morphs.** *Mitigation:* port flam3's **per-column magnitude guard** (`log(cxmag)<-10` → linear magnitude for that column; `interpolation.c:213-215`) + the zero-column angle-copy rule; constructed-degenerate-pair test in the frozen set. (flam3 has no determinant guard and no NaN guard; the magnitude switch is what keeps the polar path finite.)
