...
> velocity-matched easing**, and **loop render-once-repeat**. Engine parity is
...
> slice 2 (temporal smoothing — across-frame histogram EMA for low-spp quality)
> shipped:** export-only across-frame histogram EMA before the display pipeline,
> α-scaled by a continuous ramp (Low/Med/High; OFF at genome-default + mastering
> path), Metal via fused-chaos + atomicBuf readback (no new shader; existing fused
> cores byte-unchanged), full-window warmup on resume (resumed == never-paused).
> Engine parity + animate/export byte-identity unchanged. See the spec/plan under
> `docs/superpowers/specs|plans/2026-08-09-m6.1-temporal-smoothing*`.
