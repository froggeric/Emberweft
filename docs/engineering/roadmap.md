...
**Key deliverables:**
- `FlameKit.Framing.apply(flame:renderWidth:renderHeight:framing:)` — the orientation-aware sibling of `normalize` (conditional rotation + swapped anchor behind the degenerate-header guard); `normalize` and the renderers untouched.
