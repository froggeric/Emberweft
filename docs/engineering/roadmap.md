...
### M6 — Export Pipeline (in progress)

> **In progress.** v0.4.0 shipped the export **engine + `emberweft export` CLI**
> (H.264 + HEVC to MP4/MOV, long-form segment+concat, batch; frames byte-identical
> to `animate`; engine parity unchanged). The **GUI export sheet + progress UI**
> (this milestone's "Export progress UI with cancellation" / "progress bar" DoD
> items) is the remaining slice. See [CHANGELOG.md](../../CHANGELOG.md) and the
> M6 spec/plan under `docs/superpowers/specs|plans/2026-08-02-m6-export-pipeline-*`.
