...
> parity unchanged). **M6.1 follow-up:** export pause/resume (frame-count
> checkpointing + chunked-encode + concat) + temporal smoothing (across-frame
> histogram EMA for low-spp quality). See [CHANGELOG.md](../../CHANGELOG.md) and
> the spec/plan under `docs/superpowers/specs|plans/2026-08-0{2,3}-m6-export-*`.
