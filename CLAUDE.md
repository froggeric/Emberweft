...
- **The loop→transition boundary renders the fromSheep directly (flam3 `seqflag` shortcut).** flam3's `sheep_edge` has a `seqflag && blend==0` shortcut (flam3.c:476-477: `flam3_copy(result, &prealign[0])`) that returns A un-aligned at the boundary, skipping align+interpolate. Emberweft ports this at the CALLER layer (`Schedule.isLoopToTransitionBoundary` + `AnimateCommand.blendAt` / `PlaybackDispatcher.renderOneFrame` render the fromSheep directly at that one frame), NOT inside `Transition.blend` — because Emberweft's 1-indexed schedule emits `blend=1/N` (never 0) at the boundary, so `if t==0` there would never fire. Without it, the `.log` polar round-trip residual decorrelates spiky attractors (~21 MAD; fixed v0.1.8, boundary now ~5 MAD = rotation seam). Don't "centralize" this into `Transition.blend` without first making the schedule emit `blend=0`.
