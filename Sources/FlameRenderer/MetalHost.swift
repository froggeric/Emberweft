...
/// host-side extraction; the full device layout (483 floats = header + 52
/// varWeights + 416 varParams) is described by `floatsPerXform`/`bytesPerXform`
/// and built by the packer. The MSL `struct GPUXform` mirrors the full 483-float
/// layout field-for-field (15 scalars + `varWeights[52]` + `varParams[416]`).
public struct GPUXform {
    ...
    // (color/cs/opacity) + 52 (varWeights) + 52*8 (varParams) = 483 floats.
    ...
    public static let floatsPerXform = headerFloats + numSlots + numSlots * slotWidth   // 15+52+416 = 483
    public static let bytesPerXform = floatsPerXform * 4                    // 1932
}
...
enum MetalHost {
    ...
    /// `GPUXform.floatsPerXform` (483) floats: 15 header (a..f, pa..pf,
    /// color/colorSpeed/opacity), then 52 `varWeights` (canonical slot order,
    /// summing weights of repeated names — algebraically identical to CPU's
    /// array-order sum because variation terms commute), then 416 `varParams`
    ...
    static func packXforms(_ flame: Flame) -> [Float] {
        let order = Variations.canonicalOrder          // 52-name authority
        ...
        for (xi, xf) in flame.xforms.enumerated() {
            ...
            let wBase = base + GPUXform.headerFloats                  // 52 weights
            let pBase = wBase + GPUXform.numSlots                     // 416 params
