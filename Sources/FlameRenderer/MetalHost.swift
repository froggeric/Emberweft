...
/// host-side extraction; the full device layout (510 floats = header + 55
/// varWeights + 440 varParams) is described by `floatsPerXform`/`bytesPerXform`
/// and built by the packer. The MSL `struct GPUXform` mirrors the full 510-float
/// layout field-for-field (15 scalars + `varWeights[55]` + `varParams[440]`).
public struct GPUXform {
    ...
    // (color/cs/opacity) + 55 (varWeights) + 55*8 (varParams) = 510 floats.
    ...
    public static let floatsPerXform = headerFloats + numSlots + numSlots * slotWidth   // 15+55+440 = 510
    public static let bytesPerXform = floatsPerXform * 4                    // 2040
}
...
enum MetalHost {
    ...
    /// `GPUXform.floatsPerXform` (510) floats: 15 header (a..f, pa..pf,
    /// color/colorSpeed/opacity), then 55 `varWeights` (canonical slot order,
    /// summing weights of repeated names — algebraically identical to CPU's
    /// array-order sum because variation terms commute), then 440 `varParams`
    ...
    static func packXforms(_ flame: Flame) -> [Float] {
        let order = Variations.canonicalOrder          // 55-name authority
        ...
        for (xi, xf) in flame.xforms.enumerated() {
            ...
            let wBase = base + GPUXform.headerFloats                  // 55 weights
            let pBase = wBase + GPUXform.numSlots                     // 440 params
