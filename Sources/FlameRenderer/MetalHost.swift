...
/// host-side extraction; the full device layout (456 floats = header + 49
/// varWeights + 392 varParams) is described by `floatsPerXform`/`bytesPerXform`
/// and built by the packer. The MSL `struct GPUXform` mirrors the full 456-float
/// layout field-for-field (15 scalars + `varWeights[49]` + `varParams[392]`).
public struct GPUXform {
    ...
    // (color/cs/opacity) + 49 (varWeights) + 49*8 (varParams) = 456 floats.
    ...
    public static let floatsPerXform = headerFloats + numSlots + numSlots * slotWidth   // 15+49+392 = 456
    public static let bytesPerXform = floatsPerXform * 4                    // 1824
}
...
enum MetalHost {
    ...
    /// `GPUXform.floatsPerXform` (456) floats: 15 header (a..f, pa..pf,
    /// color/colorSpeed/opacity), then 49 `varWeights` (canonical slot order,
    /// summing weights of repeated names — algebraically identical to CPU's
    /// array-order sum because variation terms commute), then 392 `varParams`
    ...
    static func packXforms(_ flame: Flame) -> [Float] {
        let order = Variations.canonicalOrder          // 49-name authority
        ...
        for (xi, xf) in flame.xforms.enumerated() {
            ...
            let wBase = base + GPUXform.headerFloats                  // 49 weights
            let pBase = wBase + GPUXform.numSlots                     // 392 params
