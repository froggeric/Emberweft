...
/// host-side extraction; the full device layout (528 floats = header + 57
/// varWeights + 456 varParams) is described by `floatsPerXform`/`bytesPerXform`
/// and built by the packer. The MSL `struct GPUXform` mirrors the full 528-float
/// layout field-for-field (15 scalars + `varWeights[57]` + `varParams[456]`).
public struct GPUXform {
    ...
    // (color/cs/opacity) + 57 (varWeights) + 57*8 (varParams) = 528 floats.
    ...
    public static let floatsPerXform = headerFloats + numSlots + numSlots * slotWidth   // 15+57+456 = 528
    public static let bytesPerXform = floatsPerXform * 4                    // 2112
}
...
enum MetalHost {
    ...
    /// `GPUXform.floatsPerXform` (528) floats: 15 header (a..f, pa..pf,
    /// color/colorSpeed/opacity), then 57 `varWeights` (canonical slot order,
    /// summing weights of repeated names — algebraically identical to CPU's
    /// array-order sum because variation terms commute), then 456 `varParams`
    ...
    static func packXforms(_ flame: Flame) -> [Float] {
        let order = Variations.canonicalOrder          // 57-name authority
        ...
        for (xi, xf) in flame.xforms.enumerated() {
            ...
            let wBase = base + GPUXform.headerFloats                  // 57 weights
            let pBase = wBase + GPUXform.numSlots                     // 456 params
